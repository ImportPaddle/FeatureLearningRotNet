...
class ClassificationModel(Algorithm):
    def __init__(self, opt):
        ...
        self.networks['model']=self.networks['model'].cuda()
    ...
    def process_batch(self, batch, do_train=True):
        ...
        dataX_var = torch.autograd.Variable(dataX, requires_grad=(not do_train))
        ...
        record['prec1'] = float(accuracy(pre, label, topk=(1,))[0].data.cpu().numpy())
        record['loss'] = float(loss_total.data.cpu().numpy())

        # print(type(record['prec1']))
        # print(type(record['loss'] ))
